...
import { Asset, AVAILABLE_VOICES, VoiceName, TTSState, TTSCharacter, TTSDirectorConfig, TTSCharacterSettings } from '../types';
...
const TTSStudio: React.FC<TTSStudioProps> = ({ editorContent, onAddAsset, ttsState, onUpdateState }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900 text-gray-200">
      <div className="p-4 border-b border-gray-800 bg-gray-900 flex justify-between items-center">
        ...
        <div className="flex bg-gray-800 rounded-lg p-1 text-xs font-medium">
            <button 
                ...
                className={`px-3 py-1.5 rounded-md transition-all ${mode === 'single' ? 'bg-muse-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}
            >
                Narrator
            </button>
            <button 
                onClick={() => onUpdateState({ mode: 'multi' })}
                className={`px-3 py-1.5 rounded-md transition-all ${mode === 'multi' ? 'bg-muse-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}
            >
                Ensemble Cast
            </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {mode === 'single' && (
              <div className="bg-gray-850 border border-gray-700 rounded-xl p-3 flex gap-2 items-center">
                  ...
                  <button 
                      ...
                      className="bg-muse-600 hover:bg-muse-500 text-white text-xs px-3 py-1.5 rounded-lg font-medium transition-colors disabled:opacity-50 whitespace-nowrap"
                  >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div>
                      <div className="flex justify-between items-center mb-2">
                         <div className="flex gap-2 items-center flex-wrap">
                             <label className="text-xs font-medium text-gray-400 uppercase tracking-wider">Script / Timeline</label>
                             {mode === 'multi' && (
                                 <button 
                                     onClick={handleAutoFormat}
                                     disabled={isFormatting || characters.length === 0}
                                     className="text-[10px] bg-gray-800 hover:bg-gray-700 text-muse-400 px-2 py-0.5 rounded border border-gray-700 transition-colors"
                                 >
                      ...
                      {mode === 'multi' && characters.length > 0 && (
                          <div className="flex flex-wrap gap-2 mb-2 p-2 bg-gray-850 rounded-lg border border-gray-700">
                               <span className="text-[10px] uppercase font-bold text-gray-500 w-full mb-1">Quick Insert Line:</span>
                               {characters.map((char, idx) => (
                                   <button
                                     key={idx}
                                     onClick={() => insertLine(char.character)}
                                     className="flex items-center gap-1.5 px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs text-white transition-colors"
                                   >
                      ...
                      <textarea 
                          ...
                          className="w-full bg-gray-800 border border-gray-700 rounded-xl p-4 text-sm text-gray-200 focus:ring-2 focus:ring-muse-500 focus:outline-none min-h-[300px] resize-y"
                  ...
                  {mode === 'single' && (
                    <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
                        <button 
                            ...
                            className="w-full flex justify-between items-center p-3 bg-gray-850 hover:bg-gray-800 transition-colors"
                        >
                            ...
                            <svg className={`w-4 h-4 text-gray-500 transition-transform ${isDirectorMode ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        {isDirectorMode && (
                            <div className="p-4 space-y-3 bg-gray-900/50">
                                <div>
                                    <label className="text-[10px] uppercase text-gray-500 font-bold">Audio Profile (Persona)</label>
                                    <input 
                                ...
                                <div>
                                    <label className="text-[10px] uppercase text-gray-500 font-bold">The Scene</label>
                                    <textarea 
                                        value={directorConfig.scene}
                                        onChange={(e) => onUpdateState({ directorConfig: {...directorConfig, scene: e.target.value} })}
                                        className="w-full bg-gray-800 border border-gray-700 rounded p-2 text-xs text-white mt-1 h-16"
                                        placeholder="e.g. A busy London studio at night, neon lights..."
                                    />
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="text-[10px] uppercase text-gray-500 font-bold">Style / Vibe</label>
                                        <input 
                                    ...
                                    <div>
                                        <label className="text-[10px] uppercase text-gray-500 font-bold">Pacing</label>
                                        <input 
                                ...
                                <div>
                                    <label className="text-[10px] uppercase text-gray-500 font-bold">Accent</label>
                                    <input 
              ...
              <div className="bg-gray-850 rounded-xl p-4 border border-gray-800 flex flex-col h-full max-h-[600px]">
                  {mode === 'single' ? (
                      <>
                          <div className="flex justify-between items-center mb-4">
                              <label className="text-xs font-medium text-gray-400 uppercase tracking-wider">Select Voice</label>
                              <div className="text-xs text-gray-500">{AVAILABLE_VOICES.length} Voices Available</div>
                          </div>
                          <div className="flex-1 overflow-y-auto pr-2 space-y-2 custom-scrollbar">
                              {AVAILABLE_VOICES.map(v => (
                                  <div
                                    ...
                                    className={`relative cursor-pointer flex items-center justify-between p-3 rounded-lg border transition-all group ${selectedSingleVoice === v.name ? 'border-muse-500 bg-muse-500/10' : 'border-gray-700 bg-gray-800 hover:bg-gray-750'}`}
                                  >
                                      <div className="flex items-center gap-3">
                                          <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${v.gender === 'Male' ? 'bg-blue-900/40 text-blue-300' : 'bg-pink-900/40 text-pink-300'}`}>
                                              {v.name[0]}
                                          </div>
                                          <div className="text-left">
                                              <div className="text-sm font-semibold text-white">{v.name}</div>
                                              <div className="text-xs text-gray-400">{v.gender} • {v.style}</div>
                                          </div>
                                      </div>
                                      ...
                                      <div className="flex items-center gap-2">
                                          <button 
                                            ...
                                            className="p-2 rounded-full bg-gray-700 hover:bg-white hover:text-black text-gray-300 transition-all opacity-0 group-hover:opacity-100"
                                            title="Play Preview"
                                          >
                      ...
                      <>
                        <div className="flex justify-between items-center mb-4">
                            <label className="text-xs font-medium text-gray-400 uppercase tracking-wider">Cast List (Max 2 Speakers)</label>
                        </div>
                        <div className="space-y-4">
                             {characters.map((char, idx) => (
                                  <div key={idx} className="flex flex-col gap-2 bg-gray-800 p-3 rounded-lg border border-gray-700">
                                      {/* Character Header */}
                                      <div className="flex items-center gap-2">
                                      ...
                                      <div className="flex gap-2 items-center">
                                          ...
                                          <select 
                                              ...
                                              className="bg-gray-900 text-xs text-gray-300 rounded border border-gray-700 p-1.5 flex-1 focus:outline-none"
                                          >
                                              {AVAILABLE_VOICES.map(v => (
                                      ...
                                      <div className="flex gap-1 mt-1">
                                            <input 
                                                ...
                                                className="flex-1 bg-gray-900 border border-gray-700 rounded p-1.5 text-[10px] text-white focus:outline-none focus:border-muse-500"
                                            />
                                            <button 
                                                onClick={() => handleCharacterAutoSet(idx)}
                                                disabled={char.isAutoSetting || !char.autoSetInput}
                                                className="bg-gray-700 hover:bg-muse-600 text-[10px] text-white px-2 py-1 rounded transition-colors disabled:opacity-50"
                                            >
                                      ...
                                      <div className="mt-1">
                                          <button 
                                            ...
                                            className="w-full text-[10px] uppercase font-bold text-gray-400 hover:text-muse-400 border border-gray-700 hover:border-muse-500/50 bg-gray-900/50 rounded py-1.5 transition-all flex items-center justify-center gap-1"
                                          >
                                          ...
                                          {char.isExpanded && (
                                              <div className="mt-2 space-y-2 p-2 bg-gray-900 rounded border border-gray-700/50">
                                                  <div>
                                                      <label className="text-[9px] text-gray-500 uppercase">Persona / Audio Profile</label>
                                                      <input 
                                                          value={char.settings.description}
                                                          onChange={(e) => updateCharacterSettings(idx, 'description', e.target.value)}
                                                          className="w-full bg-gray-800 border border-gray-700 rounded p-1 text-xs text-gray-200 mt-0.5"
                                                          placeholder="e.g. Grumpy old man"
                                                      />
                                                  </div>
                                                  <div className="grid grid-cols-2 gap-2">
                                                      <div>
                                                          <label className="text-[9px] text-gray-500 uppercase">Style / Vibe</label>
                                                          <input 
                                                      ...
                                                      <div>
                                                          <label className="text-[9px] text-gray-500 uppercase">Pacing</label>
                                                          <input 
                                                  ...
                                                  <div>
                                                      <label className="text-[9px] text-gray-500 uppercase">Accent</label>
                                                      <input 
                              ...
                              {characters.length < 2 ? (
                                  ...
                                  <div className="text-center p-2 text-[10px] text-amber-500/80 bg-amber-900/10 rounded border border-amber-900/20">
      ...
      <div className="p-6 border-t border-gray-800 bg-gray-900 z-10">
          {lastAudioUrl && (
              <div className="mb-4 bg-muse-900/20 border border-muse-500/30 rounded-xl p-4 flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1">
                      <div className="w-10 h-10 rounded-full bg-muse-500 flex items-center justify-center text-white">
                  ...
                  <a href={lastAudioUrl} download="muse-audio.wav" className="ml-4 px-3 py-2 bg-gray-800 hover:bg-white hover:text-black rounded-lg text-xs font-medium transition-colors border border-gray-700">
                      Download WAV
                  </a>
              </div>
          )}
          ...
          <button
              ...
              className="w-full py-4 bg-gradient-to-r from-muse-600 to-purple-600 hover:from-muse-500 hover:to-purple-500 text-white rounded-xl font-bold shadow-lg shadow-purple-900/20 transition-all disabled:opacity-50 flex items-center justify-center gap-3"
          >
              {isGenerating ? (
                   <>
